...
void Alien::draw(){
    if(state==0){
        ...
        if(Game::Instance()->getP()<1){
            float posY=this->getPos().getVectorY()*(1-Game::Instance()->getP())+(this->getPos().getVectorY()+3)*Game::Instance()->getP();
            float posX=this->getPos().getVectorX()*(1-Game::Instance()->getP())+(this->getPos().getVectorX()+(sin(grados)*20))*Game::Instance()->getP();
...
void Alien::Update(){ //UPDATE DE ALIEN
    if(state == 0){
        ...
        grados=grados+0.3f;
        //updateamos la pos
        this->setPos(this->getPos().getVectorX()+(sin(grados)*10),this->getPos().getVectorY()+3);
        sprite.setPosition(position.getVectorX(),position.getVectorY());
    }
}
