...
void FactoriaNivel::readLevel(int lvl){
    ...
    //Jefe
    bBoss = level->FirstChildElement("Jefe")->GetText();
    //Escudo
    shell = level->FirstChildElement("Escudos")->GetText();
    //Municion
...
const char* FactoriaNivel::getbBoss(){
    return bBoss;
}
const char* FactoriaNivel::getShell(){
    return shell;
}
