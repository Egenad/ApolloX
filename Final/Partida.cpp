...
void Partida::update(){
    if(state==0){
        ...
        if(ship->getPosition().getVectorY()<=(-length-860)){
            if(mode==0){
                state=1;
                Nave::Instance()->setState(2);
                Nave::Instance()->setMunition(1);
                this->sacarAliensPantalla();
                text[4].setString("normal");
        ...
        if(wait.getElapsedTimeAsSeconds() > 2){
            ship->update2();
            ship->disparar(texture);
            if(waitEnd==false){
                ship->ismovingDown(false);
...
void Partida::sacarAliensPantalla(){
    for(int i=0; i<t1; i++){
        aliens[i].setState();
    }
    for(int i=0; i<t2; i++){
...
void Partida::Init(int i){
    ...
    if(t4 == 1){
        ...
        jefe->setStrategy(new StrategyA2());
    }else if(t4 == 3){
        int randing = rand() % 10 + 1;
        if(randing > 5){
            jefe->setStrategy(new StrategyA());  
        }else{
            jefe->setStrategy(new StrategyA2());
...
void Partida::reduceLifeBarBoss(int life){
    int width=(life*219)/jefe->getMaxLife();;
...
void Partida::setViewtoShip(){
    if(state == 0){
        ...
        view.setCenter(1535/2,(-length-860)-360);
    }
    m2D::RenderWindow::Instance()->setView(view);
}
...
void Partida::setMunicion(int n){
        if(n==1){
            text[4].setString("normal");
        }else if(n==2){
