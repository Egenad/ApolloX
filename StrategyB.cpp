...
void StrategyB::execute(m2D::Sprite& sprite, std::vector<Bala*>& balas_jefe, m2D::Texture& texture){
    if(reset_fase == true){//paso de la fase A a la B
        ...
        if(segundas_balas.getElapsedTimeAsSeconds() > 2 && namber < 5){
            ...
            if(lastmove == 0){
                if(Nave::Instance()->getPosition().getVectorX() > 765){
                    result = (90-result) + 90 ;
                }
            }else{
                if(lastmove == 1){
                    if(Nave::Instance()->getPosition().getVectorX() > 958){
                        result = (90-result) + 90 ;
                    }
                }else{
                    if(lastmove == 2){
                        if(Nave::Instance()->getPosition().getVectorX() > 482){
                            result = (90-result) + 90 ;
                        }
                    }
                }
        ...
        if(oleada == true){
            for(int i=0; i<balas_jefe.size(); i++){
                if(balas_jefe[i]->getTipo() == 2){
                    ...
                    balas_jefe[i]->disparar_angulo(2);
                    if(balas_jefe[i]->getX() < 340 || balas_jefe[i]->getX() > 1190){
                        delete balas_jefe[i];
                        balas_jefe.erase(balas_jefe.begin() + i);
                        // std::cout << "Eliminado" << std::endl;
                    }
                    if(balas_jefe[i]->returnShape().getGlobalBounds().intersects(Nave::Instance()->returnShape().getGlobalBounds())){
